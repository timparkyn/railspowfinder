  ...
  def get_forecast(selections)
    ...
    selections.map do |selection|
      ...
      response = JSON.parse(raw_response.to_json, object_class: OpenStruct)
        ...
        if forecast[:observations][:precip_today].to_f < 0
          forecast[:observations][:precip_today] = '0'
        end

        if forecast[:observations][:precip_1hr].to_f < 0
          forecast[:observations][:precip_1hr] = '0'
        end
